...
import com.seasia.prism.newsfeed.displaynewsfeed.view.NewsFeedFragment.Companion.videoOpenStatus
...
//                arrayList.addAll(mList.get(position).lstTaggedUsers!!)
            ...
            holder.img!!.setOnClickListener {
                if (!UtilsFunctions.isNetworkAvailable(App.app)) {
                    ...
                    if (mList.get(position).PostedById != null) {
                        if (videoOpenStatus == 0) {
                            var intent = Intent(context.activity, ProfileActivity::class.java)
                            ...
                            intent.putExtra("profileImage", mList.get(position).PostedByImageUrl)
                            intent.putExtra("userName", mList.get(position).PostedBy)
                            intent.putExtra("status", mList.get(position).Bio)
            ...
            holder.share!!.setOnClickListener {
                if (mList.get(position).lstDocuments != null && mList.get(position).lstDocuments!!.get(
                        0
                    ).Type != null
                ) {
                    var isTYpe = mList.get(position).lstDocuments!!.get(0).Type
