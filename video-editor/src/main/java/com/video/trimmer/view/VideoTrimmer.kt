...
class VideoTrimmer @JvmOverloads constructor(context: Context, attrs: AttributeSet, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    fun onSaveClicked() {
        ...
        var outPath=  Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES).absolutePath + File.separator + "VID_" +  SimpleDateFormat("yyyyMMdd_HHmmss", getLocale()).format( Date()) + ".mp4";
        ...
        //
//        val root = File(outPath)
//        root.mkdirs()
        val outputFileUri = Uri.parse(outPath)
       // val outPutPath = getAndroidMoviesFolder()!!.getAbsolutePath() + "/" + SimpleDateFormat("yyyyMM_dd-HHmmss").format(Date()) + "cameraRecorder.mp4"
...
//        val outputFileUri = Uri.fromFile(File(root, "t_${Calendar.getInstance().timeInMillis}_" + file.nameWithoutExtension + ".mp4"))
     //   val outPutPath = RealPathUtil.realPathFromUriApi19(context, outputFileUri);
        ...
        Log.d("SOURCE12345", file.path)
      //  Log.d("DESTINATION", outPutPath)
        ...
        VideoOptions(context).trimVideo(TrimVideoUtils.stringForTime(mStartPosition), TrimVideoUtils.stringForTime(mEndPosition), file.path, outPath!!, outputFileUri, mOnTrimVideoListener)
    }
    ...
    fun setDestinationPath(): VideoTrimmer {
